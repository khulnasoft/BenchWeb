defmodule BenchWeb.Handlers.Update do
  @moduledoc """
  ...
  def handle(conn) do
    number_of_queries = BenchWeb.Handlers.Helpers.parse_queries(conn, "queries")
      ...
      |> Enum.map(
        &Task.async(fn ->
          %{randomnumber: current_randomnumber} =
            world = BenchWeb.Repo.get(BenchWeb.Models.World, &1)
