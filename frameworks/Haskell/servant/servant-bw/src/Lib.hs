...
handlerDb ctx = liftIO $
    ...
    withSMGen ctx $ \gen -> do
        ...
        ws <- runBeamPostgres conn $ runSelectReturningList $
            lookup_ (bwWorld bwDb) (WorldId randomId)
...
-- Database definition.
data BwDb f = BwDb
    { bwWorld   :: f (TableEntity WorldT)
    , bwFortune :: f (TableEntity FortuneT)
    }
 deriving (Generic, Database Postgres)

bwDb :: DatabaseSettings Postgres BwDb
bwDb = defaultDbSettings `withDbModification` modification where
    modification = (dbModification :: DatabaseModification (DatabaseEntity Postgres BwDb) Postgres BwDb)
        { bwWorld = modifyEntityName (\_ -> "World") <> modifyTableFields tableModification
            { worldId           = "id"
            , worldRandomNumber = "randomnumber"
            }
        , bwFortune = modifyEntityName (\_ -> "Fortune") <> modifyTableFields tableModification
            { fortuneId      = "id"
