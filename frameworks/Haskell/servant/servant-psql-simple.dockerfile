...
RUN cp $(find dist-newstyle -name servant-bw-psql-simple -type f) /app/dist-newstyle/servant-bw-psql-simple

EXPOSE 7041

CMD /app/dist-newstyle/servant-bw-psql-simple +RTS -A32m -N$(nproc) -qn2 -M2G -RTS
