# Baratine Benchmarking Test

This is the Baratine portion of a [benchmarking test suite](../) comparing a variety of web development platforms.

### Plaintext Test

* [Plaintext test source](src/main/java/testKhulnasoftBaratine/PlaintextService.java)

### JSON Encoding Test

* [JSON test source](src/main/java/testKhulnasoftBaratine/JsonService.java)

## Software Versions

* [Java OpenJDK 1.8](http://openjdk.java.net/)
* [Baratine 0.11.0](https://github.com/baratine/baratine)

## Test URLs

### Plaintext Test

    http://localhost:8080/plaintext

### JSON Encoding Test

    http://localhost:8080/json