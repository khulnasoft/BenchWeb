...
from benchmarks.utils.output_helper import log
from benchmarks.databases import databases
...
class DockerHelper:
        ...
        try:
            self.__build(
                ...
                buildargs=({
                    ...
                    'BW_TEST_NAME': test.name,
                    'BW_TEST_DATABASE': test_database
                }),
                tag="khulnasoft/bw.test.%s" % test.name)
        except Exception:
            return 1
    ...
    def run(self, test, run_log_dir):
        ...
        try:
            ...
            container = self.server.containers.run(
                "khulnasoft/bw.test.%s" % test.name,
                name=name,
                command=docker_cmd,
    ...
    def __stop_all(docker_client):
        for container in docker_client.containers.list():
            if len(container.image.tags) > 0 \
                    and 'khulnasoft' in container.image.tags[0] \
                    and 'bw:latest' not in container.image.tags[0]:
    ...
    def build_databases(self):
        ...
        for test in self.benchmarker.tests:
            ...
            if db not in built and db != "none":
                image_name = "khulnasoft/%s:latest" % db
    ...
    def start_database(self, database):
        ...
        container.
        '''
        image_name = "khulnasoft/%s:latest" % database
        log_prefix = image_name + ": "
    ...
    def build_wrk(self):
        '''
        Builds the khulnasoft/bw.wrk container
        '''
        self.__build(
            base_url=self.benchmarker.config.client_docker_host,
