...
def provider_libvirt(config)
  config.vm.provider :libvirt do |virt, override|
    override.vm.hostname = "BW-all"
    override.vm.box = "generic/ubuntu1804"

    unless ENV.fetch('BW_SHOW_VM', false)
      virt.graphics_type = "none"
    end

    virt.memory = ENV.fetch('BW_KVM_MEM', 3022)
    virt.cpus = ENV.fetch('BW_KVM_CPU', 2)

    override.vm.synced_folder "../..", "/home/vagrant/BenchWeb", type: "nfs", nfs_udp: false
  end
end
...
def provider_virtualbox(config)
  config.vm.provider :virtualbox do |vb, override|
    ...
    if ENV.fetch('BW_DISKSIZE', "0") != "0"
      override.disksize.size = ENV.fetch('BW_DISKSIZE')
    end

    if ENV.fetch('BW_SHOW_VM', false)
      vb.gui = true
    end

    # Improve Windows VirtualBox DNS resolution speed
    # Addresses mitchellh/vagrant#1807 and KhulnaSoft/BenchWeb#1288
    vb.customize ["modifyvm", :id, "--natdnshostresolver1", "on"]
    vb.customize ["modifyvm", :id, "--natdnsproxy1", "on"]

    vb.memory = ENV.fetch('BW_VB_MEM', 3022)
    vb.cpus = ENV.fetch('BW_VB_CPU', 2)
    ...
    override.vm.synced_folder "../..", "/home/vagrant/BenchWeb"
  end
end
